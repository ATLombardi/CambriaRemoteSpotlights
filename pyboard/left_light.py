...
SIDE_TAG   = 'L'        # we're the right-side light
LIM_CW_A   = -5000      # horizontal range max
LIM_CCW_A  =  2000      # horizontal range min
LIM_CW_B  =  -4000      # vertical range max
LIM_CCW_B  =   500      # vertical range min
...
def find_zeroes ():
  ...
  speed    = -7                 # initial direction is CCW
...
#      print('A: ',setpoint_a,' B: ',setpoint_b)
    ...
    if (setpoint_a <= LIM_CW_A):
      setpoint_a = LIM_CW_A
    elif (setpoint_a >= LIM_CCW_A):
      setpoint_a = LIM_CCW_A

    # soft limits to prevent going beyond safe ranges
    if (setpoint_b <= LIM_CW_B):
      setpoint_b = LIM_CW_B
    elif (setpoint_b >= LIM_CCW_B):
      setpoint_b = LIM_CCW_B
